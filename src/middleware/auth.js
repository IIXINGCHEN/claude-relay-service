...
const securityMiddleware = (req, res, next) => {
  ...
  // 只在 HTTPS 或 localhost 环境下设置严格的安全策略
  if (isLocalhost || isHttps) {
    res.setHeader('Cross-Origin-Opener-Policy', 'same-origin')
    res.setHeader('Cross-Origin-Resource-Policy', 'same-origin')
    res.setHeader('Origin-Agent-Cluster', '?1')
  } else {
    // HTTP 非localhost环境，放宽策略避免问题
    res.setHeader('Cross-Origin-Resource-Policy', 'cross-origin')
  }

  // Content Security Policy (适用于web界面和管理界面)
  if (req.path.startsWith('/web') || req.path.startsWith('/admin-next') || req.path === '/') {
    const cspDirectives = [
      ...
      "font-src 'self' https://cdnjs.cloudflare.com https://cdn.bootcdn.net https://fonts.gstatic.com data:",
